...
Data_out = cell(num_future+2,5);
Data_out(1,:) = { "", "#samples" , "Mean (ms)", "Std (ms)", "Median (ms)" };
Data_out(2,:) = {
  "Stand-by" , ...
...
for i=1:num_future
  ...
  FutureTimeDataVizLabel =     ...
    [ FutureTimeDataVizLabel ; ...
      repmat( sprintf("M = %d", FutureSize(i) ) , length(FutureTimeData{i}) , 1 ) ];
end
...
xlabel('Future size (M) \rightarrow')
ylabel('time (ms) \rightarrow')
title('Stand-by VS Controller following trajectory with a future size M')
