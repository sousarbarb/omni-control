function [Xfiltered] = nonCausalMedianFilter(X,windowSize)
%NONCAUSALMEDIANFILTER
%
%[XFILTERED] = NONCAUSALMEDIANFILTER(X,WINDOWSIZE)
%
%Inputs:
...
%  XFILTERED: matrix containing the filtered data.
  ...
  if isOdd
    for i=1:numExp
      ...
      for j=1:itf
        if (j < windowSize/2)
          Xfiltered(i,j) = median(X(i,1:j+sideLength));
          sideLength = sideLength + 1;
        elseif (itf-j+1 < windowSize/2)
          sideLength = sideLength - 1;
          Xfiltered(i,j) = median(X(i,j-sideLength:end));
        else
          Xfiltered(i,j) = median(X(i,j-sideLength:j+sideLength));
        end
      end
    end
    ...
    for i=1:numExp
      ...
      for j=1:itf
        ...
        elseif (itf-j < windowSize/2)
          ...
          Xfiltered(i,j) = ...
            median([ 0.5*( X(i,j-sideLength) + X(i,j+sideLength) ) , ...
